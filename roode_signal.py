...
roode_signal.py
───────────────
Roode Radioactive Signal 주입 모듈.
...
class RoodeSignal:
    """
    Roode 플랫폼의 Radioactive Signal 주입기.
    ...
    def __init__(self, secret_seed=42, device=None):
        ...
        # Roode 고유 signal direction (비밀 시드 / v1)
    ...
    def inject_signal(
        ...
        """
        이미지에 Roode radioactive signal을 심는다.
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python roode_signal.py <input_image> <output_image>")
        sys.exit(1)
    sig    = RoodeSignal()
